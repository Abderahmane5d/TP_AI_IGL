...
class UserModel(models.Model):
    ...
    adresse = models.TextField(null=False, blank=True) #the db can't have an instance of this model without this attribute
...
class CustomAccountManager(BaseUserManager):
    ...
    def create_user(self, email, nom, prenom, adresse, telephone, password, **otherfields):
        ...
        user = self.model(email=email, nom=nom, prenom=prenom, adresse=adresse, telephone=telephone, **otherfields)
        user.set_password(password)
        user.save()
        return user

class Utilisateur(AbstractBaseUser, PermissionsMixin): # ajouter le nom et prenom ou bien le username
    nom = models.CharField(max_length=50, blank=True)
    prenom = models.CharField(max_length=100, blank=True)
    adresse = models.TextField(null=False, blank=True) # the db can't have an instance of this model without this attribute
    ...
    is_active = models.BooleanField(default=False) # for a secondary check (email sent to user they click and that activate the user and they can login)
...
class Favories(models.Model):
    user = models.OneToOneField(Utilisateur, on_delete= models.CASCADE, related_name='Favories') # related_name : bach ndiro l'accès mel jiha lokhra
...
class Message(models.Model):
    ...
    destination = models.ForeignKey(Utilisateur, on_delete=models.CASCADE, related_name='notification')
...
class AI(models.Model):
    ...
    type = models.CharField(choices=TypeChoices.choices, max_length=22) # null = False (default)
    category = models.CharField(choices=CategoryChoices.choices,max_length=16) # null = False (default)
    location = models.ForeignKey(Location, on_delete=models.CASCADE) # à revoir
    owner = models.ForeignKey(Utilisateur, on_delete=models.CASCADE, related_name='anonces')
    created = models.DateTimeField(auto_now_add=True)
